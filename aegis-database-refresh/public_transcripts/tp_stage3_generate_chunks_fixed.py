...
2. Natural boundaries - Does this sentence start a new question, answer, or topic?
3. Semantic completeness - Would adding this sentence make the chunk too long or unfocused?
4. Context flow - Does the sentence logically belong with the current chunk?
"""
...
def generate_chunk_metadata(
    ...
    prompt = f"""
Analyze this earnings call transcript chunk and extract metadata using the surrounding context.

Context from surrounding chunks:
{json.dumps(context_chunks, indent=2)}

Extract the following metadata for the target chunk:
1. Primary speaker - Identify who is speaking in this chunk. Look for speaker indicators in surrounding chunks if not clear in the target chunk.
2. Key tags - 3-5 keywords/phrases that capture the main concepts discussed
3. Main topics - 2-3 thematic topics being addressed
4. Related chunks - IDs of other chunks that discuss similar topics or are contextually related
5. Supporting context - Important information from other chunks that helps understand this chunk

Remember to check the surrounding chunks for speaker identification, as the speaker may be introduced before the target chunk.
"""
