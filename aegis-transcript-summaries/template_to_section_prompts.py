            ...
            if checkbox.has_attr('checked') and not checkbox.has_attr('disabled'):
                ...
                if widget_type and widget_type in self.widget_templates:
                    enabled_widgets.append({
                        'type': widget_type,
                        'description': self.widget_templates[widget_type]['description'],
                        'instructions': self.widget_templates[widget_type]['instructions']
                    })
    ...
    def generate_individual_section_prompts(self, config_data: Dict) -> List[Dict]:
        ...
        for section in config_data['sections']:
            ...
            section_prompts.append({
                ...
                'section_id': section['name'].lower().replace(' ', '_').replace('-', '_'),
                'prompt': prompt,
    ...
    def _generate_single_section_prompt(self, section: Dict) -> str:
        ...
        if section['enabled_widgets']:
            for widget in section['enabled_widgets']:
                widget_requirements.append(f'<widget name="{widget["type"]}">{widget["instructions"]}</widget>')
        
        widget_content = "\n".join(widget_requirements) if widget_requirements else "<widget name=\"summary\">Provide structured summary with key points and context</widget>"
...
{widget_content}
    ...
    def save_section_prompts(self, section_prompts: List[Dict], output_dir: str = None) -> List[str]:
...
"""
    ...
    def convert(self, output_dir: str = None) -> List[str]:
        ...
        print(f"Generated {len(section_prompts)} individual section prompt markdown files")
        print(f"Saved to directory: {Path(saved_files[0]).parent}")
        for section in section_prompts:
            print(f"  - {section['section_name']}: {len(section['enabled_widgets'])} widgets")
