...
class FinalSectionGenerator:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        # Set up paths
        self.input_folder = Path(self.config["output_folder"])  # Read from research_plans folder
        self.output_folder = Path(self.config.get("final_sections_folder", "final_sections"))
    ...
    def generate_section_content_prompt(self, section_name: str, research_plan: str, 
                                      transcript_text: str, prior_sections: str = "") -> str:
        """Generate prompt for creating final section content."""
        
        prior_context = f"\n\n## Prior Completed Sections\n{prior_sections}" if prior_sections else ""
        
        prompt = f"""You are creating the final formatted section content for an earnings call analysis.
...
## Instructions
Execute the research plan above to create the final formatted section content. Use the research plan as your guide for:
- What specific content to extract from the transcript
- How to structure and organize the information
- Which quotes and data points to highlight
- How to present the information clearly

Extract and format the content according to the research plan's specifications. Ensure the section is comprehensive, well-organized, and follows the guidance provided in the research plan.{prior_context}
...
{transcript_text}"""
    ...
    def process_analysis_folder(self, analysis_folder: Path) -> Path:
        ...
        for i, plan_info in enumerate(research_plans, 1):
            self.logger.info(f"Creating final section {i}/{len(research_plans)}: {plan_info['section_name']}")
            ...
            try:
                ...
                prompt_file = self.current_analysis_folder / f"{plan_info['section_id']}_content_prompt.txt"
                ...
                # Save final section
                section_file = self.current_analysis_folder / f"{plan_info['section_id']}_final_section.md"
                ...
                self.logger.info(f"Final section completed: {plan_info['section_name']}")
                
            except Exception as e:
                self.logger.error(f"Failed to create final section for {plan_info['section_name']}: {e}")
                raise
        
        # Generate final complete report
        self._generate_complete_report(transcript_text)
        
        self.logger.info(f"Final sections completed. Results in: {self.current_analysis_folder}")
...
"""
        ...
        for i, section in enumerate(self.completed_sections, 1):
            report_content += f"## {i}. {section['section_name']}\n\n{section['content']}\n\n---\n\n"
...
"""
    ...
    def run(self):
        ...
        try:
            ...
            self.logger.info(f"Found {len(analysis_folders)} analysis folder(s) to process")
